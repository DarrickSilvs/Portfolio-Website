...
const Home: React.FC = () => {
    ...
    return (
        <div className="relative w-full h-full flex items-center justify-center font-mono overflow-hidden">
            {/* ASCII background */}
                <pre className="absolute inset-0 text-green-600 opacity-30 whitespace-pre-wrap text-[10px] sm:text-xs leading-tight sm:leading-none animate-pulse pointer-events-none select-none overflow-hidden text-center mx-auto px-2">
                    {paddedAscii}
                </pre>
        
            {/* Content */}
            <div className="relative z-10 text-center text-green-400 text-base sm:text-xl space-y-4 px-4 sm:px-6">
                <p className="text-xs sm:text-sm">
                    Last login: {lastLogin} on /dev/portfolio
                </p>
                <h1 className="text-2xl sm:text-3xl text-green-500">
